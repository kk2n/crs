...
class AddForm extends Component {
  ...
  async componentDidMount() {
    ...
    this.setState({
      guizhe: [
        this.props.info?.rule.signingDay,
        this.props.info?.rule.lessonHour,
        this.props.info?.rule.totalLessonHour
      ]
    })
    await this.props.form.setFieldsValue({
      name: this.props.info?.name,
      type: { key: this.props.info?.type, label: this.props.info?.typeName },
      imageUrl: this.props.info?.imageUrl
    })
    await this.props.form.setFieldsValue({
      rule: this.props.info?.rule
        ? [this.props.info?.rule?.signingDay, this.props.info?.rule?.lessonHour, this.props.info?.rule?.totalLessonHour]
        : undefined
    })
    if (this.props.info?.imageUrl) {
      this.setState({ imageUrlArr: this.props.info?.imageUrl })
    }
    ...
    //还原表格
    if (this.props.info?.priceList?.length) {
      let tableData = this.state.tableData
      this.props.info?.priceList.forEach((a, aa) => {
        tableData[aa].danji = a.unitPrice
        toArr(this.props.info?.priceList?.[aa]?.details).forEach((b, bb) => {
          tableData[aa]['zj' + (bb + 1)] = b.totalPrice
          tableData[aa]['youhui' + (bb + 1)] = b.maxDiscountAmount
        })
      })
      this.setState({ tableData })
    }
    if (this.props.info?.priceList?.[0]?.details?.length) {
      this.setState({ index: this.props.info?.priceList?.[0]?.details?.length })
      //还原课时包
      let columns = this.state.columns
      let ksbDataHY = this.state.ksbData
      toArr(this.props.info?.priceList?.[0]?.details).forEach((a, aa) => {
        ksbDataHY[aa] = a.lessonHour
        columns[aa + 2] = {
          title: this.slCom(a.lessonHour, aa + 1),
          children: [
            {
              title: '总价',
              dataIndex: 'zj' + (aa + 1),
      ...
      this.setState({ columns, ksbData: ksbDataHY })
    }
  }
  handleSubmit = e => {
    e.preventDefault()
    this.props.form.validateFieldsAndScroll(async (err, values) => {
      ...
      let params = {
        ...
        rule: {
          signingDay: this.state.guizhe?.[0],
          lessonHour: this.state.guizhe?.[1],
          totalLessonHour: this.state.guizhe?.[2]
        },
        priceList
      }
      // console.log('values.rule', params)
    ...
    return (
      <Form {...formItemLayout} onSubmit={this.handleSubmit}>
        <Form.Item label="名称">
          ...
          })(<Input style={{ width: 240 }} maxLength={30} />)}
        </Form.Item>
        <Form.Item label="类型">
          ...
          <Form.Item label="规则">
            {getFieldDecorator('rule')(
              <div>
                1、新签学员在签约后{' '}
                <InputNumber
                  defaultValue={this.props.info?.rule?.signingDay}
                  min={0}
                  max={999}
                  onChange={v => this.onChange(v, 'one')}
                />
                天内再续费，购买 >{' '}
                <InputNumber
                  defaultValue={this.props.info?.rule?.lessonHour}
                  min={0}
                  max={999}
                  onChange={v => this.onChange(v, 'two')}
                />
                时都可以享受当前价格表最大的优惠
                <br />
                2、学员累计已购买的常规课时数达到
                <InputNumber
                  defaultValue={this.props.info?.rule?.totalLessonHour}
                  min={0}
                  max={999}
                  onChange={v => this.onChange(v, 'three')}
                />
                课时后，可永久享受当前价格表最大的优惠
              </div>
            )}
        ...
        <Form.Item label="图片">
          {getFieldDecorator('imageUrl', { rules: [{ required: true, message: '请上传' }] })(
            <>
              ...
              />
              <div style={{ clear: 'both' }}>支持jpg、png，大小1MB以内</div>
            </>
          )}
        </Form.Item>
