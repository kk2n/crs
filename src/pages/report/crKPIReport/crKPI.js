...
model({
  ...
  async daochu() {
    ...
    let { status } = await API.post('/reportservice/list/kpi/cr', params)
    if (!status) return false
    let { data } = await downAPI.post('/reportservice/export/list/kpi/cr', params)
    if (data?.type !== 'text/xml') return !message.error('出现错误！')
    download(data, `客服KPI报表.xls`)
  }
})
export default () => {
  let m = useModel('CRKPIReport')
  return (
    <div className={'CRKPIReport'}>
      <Filter m={m} />
