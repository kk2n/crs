...
model({
  ...
  async daochu() {
    ...
    let { status } = await API.post('/reportservice/list/kpi/cc', params)
    if (!status) return false
    let { data } = await downAPI.post('/reportservice/export/list/kpi/cc', params)
    if (data?.type !== 'text/xml') return !message.error('出现错误！')
    download(data, `销售KPI报表.xls`)
  }
})
export default () => {
  let m = useModel('CCKPIReport')
  return (
    <div className={'CCKPIReport'}>
      <Filter m={m} />
